...
interface CentralHeatingFilterProps {
  isCollapsed?: boolean;
  onChange: (value: string, checked: boolean) => void;
  resetKey?: number; // Add resetKey prop
}

const CentralHeatingFilter: React.FC<CentralHeatingFilterProps> = ({
  isCollapsed = true,
  onChange,
  ...
  useEffect(() => {
    const initialStates = heatingTypes.reduce((acc, type) => {
      acc[type.toUpperCase().replace(" ", "_")] = false;
      return acc;
          ...
          {heatingTypes.map((type) => (
            <div className="flex-row mb-2" key={type}>
              <input
                ...
                className="mr-2 mb-1"
                checked={
                  checkedStates[type.toUpperCase().replace(" ", "_")] || false
...
export default CentralHeatingFilter;
